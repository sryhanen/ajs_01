...
angular.module('zeppelinWebApp.paragraph').directive('resizable', [ResizableDirective]);
...
function ResizableDirective() {
  ...
  const addEvent = function(config) {
    const removeEventByID = function(id) {
      const events = jQuery.data(config.element, 'events')[config.eventType];
  ...
  return {
    ...
    link: function postLink(scope, elem, attrs) {
      attrs.$observe('resize', function(resize) {
        const resetResize = function(elem, resize) {
          const colStep = window.innerWidth / 12;
          elem.off('resizestop');
          const conf = angular.copy(resizableConfig);
