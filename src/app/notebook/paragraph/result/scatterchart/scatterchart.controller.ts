...
import ScatterchartVisualization from './visualization-scatterchart';

angular.module('zeppelinWebApp.vizScatterchart')
  .directive('callback', [callbackDir]
  )
  .controller('ScatterchartCtrl', [
    '$scope',
    'vizRegisterService',
    'columnselectorTransformationsService',
    'tableParserService',
    ScatterchartCtrl
  ]);
...
function ScatterchartCtrl(
  ...
  const refresh = function (newData, newConfig){
    ...
    $scope.config = columnselectorTransformationsService.getColumnselectorSetting($scope.vizConfig);
    transformed = columnselectorTransformationsService.columnselectorTransform(tableData, $scope.vizConfig);
  ...
  */
  $scope.initResult = function (id, index, data,  vizConfig) {
    ...
    $scope.save = function (collection){
      ...
      if(collection){
        ...
        const newCollection = {};
        collection.map(zone => {
          newCollection[zone.id] = zone.content[0];
        });

        viz.config = newCollection;
  ...
  $scope.initViz = function (element) {
    ...
    if(!viz) {
      ...
      element[0].id = vizID;
      viz = new ScatterchartVisualization(element, $scope.vizConfig);
      viz.render(transformed);
