...
angular.module('zeppelinWebApp.vizScatterchart', [])
  .service('vizScatterchartService', [
    'vizRegisterService',
...
function vizScatterchartService(
  vizRegisterService,
) {

  const self = this;
  self.inclusion = '/app/notebook/paragraph/result/scatterchart/scatterchart.html';
  self.controllerName = 'ScatterchartCtrl';

  self.hasSettings = true;
  self.compatibleList = [
    DisplayType.TABLE,
  ];
  self.id = 'scatterChart';
  self.name = 'Scatter Chart';
  self.buttonIcon = '<i class="cf cf-scatter-chart"></i>';
