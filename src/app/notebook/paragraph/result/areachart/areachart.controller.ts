...
import AreachartVisualization from './visualization-areachart';

angular.module('zeppelinWebApp.vizAreachart')
...
function AreachartCtrl(
  ...
  */
  $scope.initResult = function (id, index, data, vizConfig) {
    ...
    //callback used by the setting panel to save the config
  ...
  $scope.initViz = function (element) {
    ...
    if(!viz) {
      ...
      element[0].id = vizID;
      viz = new AreachartVisualization(element, $scope.vizConfig);
      viz.render(transformed);
