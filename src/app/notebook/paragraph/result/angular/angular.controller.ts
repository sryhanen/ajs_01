 ...
 */
angular.module('zeppelinWebApp.vizAngular')
...
function AngularCtrl(
  ...
  const render = function (){
    const id = $scope.paragraphID;
    try {
      const paragraphScope = noteVarShareService.get(`${id}_paragraphScope`);
      elem.html($scope.data);
      $compile(elem.contents())(paragraphScope);
    }catch (e) {
      console.warn('Angular element failed to render with: ', e);
      elem.html(`${e.stack}`);
    }
  };
