...
angular.module('zeppelinWebApp.comNoteImport', []).controller('NoteImportCtrl', ['$scope', '$timeout', 'websocketMsgSrv', NoteImportCtrl]);

function NoteImportCtrl($scope,
                        $timeout,
                        websocketMsgSrv,
                        ) {
  ...
  $scope.importFile = function(element) {
    ...
    const file = $scope.note.importFile;
  ...
  vm.processImportJson = function(result) {
    if (typeof result !== 'object') {
      try {
        ...
        $scope.note.errorText = 'JSON parsing failed';
        $scope.$apply();
        return;
      }
    }
  ...
  $scope.$watch('importingFile', function (newVal, oldVal) {

    if (newVal){
      $scope.importFile(newVal);
    }
  });
}
