...
angular.module('zeppelinWebApp.comArrayOrdering', [])
  .service('arrayOrderingSrv', ['TRASH_FOLDER_ID', ArrayOrderingService])
  .constant('TRASH_FOLDER_ID', '~Trash');

function ArrayOrderingService(TRASH_FOLDER_ID) {

  const arrayOrderingSrv = this;
  ...
  this.noteComparator = function(v1, v2) {
    ...
    const noteName1 = arrayOrderingSrv.getNoteName(note1);
    const noteName2 = arrayOrderingSrv.getNoteName(note2);
