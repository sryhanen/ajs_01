...
angular.module('zeppelinWebApp.comNoteCreate')
  .service('noteCreateService', ['$rootScope', '$timeout', 'websocketMsgSrv', NoteCreateService]);

function NoteCreateService($rootScope, $timeout, websocketMsgSrv) {
  ...
  self.init = function(path) {
    websocketMsgSrv.getInterpreterSettings();
    if(path){
      self.path = path;
    }
    self.nameSet = false;
