...
angular.module('zeppelinWebApp').service('websocketMsgSrv', ['$rootScope', 'websocketEvents', WebsocketMessageService]);

export function WebsocketMessageService($rootScope, websocketEvents) {
  ...
  return {
    ...
    paragraphUpdateResult: function(noteId, paragraphId, draw, start, length, search) {
      websocketEvents.sendNewEvent({op: 'PARAGRAPH_UPDATE_RESULT',
        data: {
          noteId: noteId,
          paragraphId: paragraphId,
          draw: draw,
