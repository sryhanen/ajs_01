...
import {ToggleTitleView} from './toggleTitleView';

describe('ToggleOutputView functional test', () => {
  let fixture: ComponentFixture<ToggleTitleView>;
  ...
  beforeEach(async () => {
    ...
    const renderResult = await render(ToggleTitleView, {
      inputs: {
  ...
  describe('Birth', () => {
    ...
    it('Should have specific title', () => {
      expect(screen.getByTitle('Show title')).toBeDefined();
    });
  });

  describe('Toggling title', () => {
    let requestSpy;
    beforeEach(() => {
    ...
    it('Should have changed title and class name', () => {
      expect(screen.getByTitle('Hide title')).toBeDefined();
    });
  });
});
