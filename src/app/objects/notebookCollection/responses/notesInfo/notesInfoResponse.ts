...
export class NotesInfoResponse implements Response{
  ...
  constructor(notebooks: WritableSignal<Map<string, Notebook>>, channel: Channel) {
  ...
  response(data: object) {
    ...
    if(operation === 'NOTES_INFO'){
      ...
      notes.forEach(note => {
        const notebook = new NotebookImpl(this._channel, note);
        this._notebooks.update((notebooks) => {
          return notebooks.set(notebook.id(), notebook);
        });
      });
    }
  }
