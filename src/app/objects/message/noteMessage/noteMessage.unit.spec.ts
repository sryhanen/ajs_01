...
import {CreateFakeNotebookIndex} from '../../../../test/fakes/notebookIndex/fakeNotebookIndexFactory';

describe('NoteMessage unit test', () => {
  const notebookData = {
    name:'name',
    id:'id',
    paragraphs:[]
  };
  let noteMessage = new NoteMessageImpl(new MessageImpl(new WebSocketPayloadImpl({
    op:'NOTE',
    data:notebookData
  })));
  ...
  it('Should throw if operation is invalid', () => {
    noteMessage = new NoteMessageImpl(new MessageImpl(new WebSocketPayloadImpl({
      op:'NOT',
      data:notebookData
    })));
