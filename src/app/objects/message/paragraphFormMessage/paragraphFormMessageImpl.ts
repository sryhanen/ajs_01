...
export class ParagraphFormMessageImpl implements ParagraphFormMessage {
  ...
  form(): { type: string; name: string; value: unknown }[] {
    ...
    const form:[] = messageData.getProperty('form', 'object');
    form.forEach((formField) => {
      this.validateFormField(formField, 'type');
      this.validateFormField(formField, 'name');
      this.validateFormField(formField, 'value');
    });
    return form;
  }
