...
import {CreateFakeNotebookCollection} from '../../../../test/fakes/notebookCollection/fakeNotebookCollectionFactory';
...
describe('NotesInfoMessage unit test', () => {
  ...
  let notesInfoMessage = new NotesInfoMessageImpl(new MessageImpl(new WebSocketPayloadImpl({
    op:'NOTES_INFO',
    data:{
      notes:notes
    }
  ...
  it('Should throw if operation is invalid', () => {
    notesInfoMessage = new NotesInfoMessageImpl(new MessageImpl(new WebSocketPayloadImpl({
      op:'NOTE_INFO',
      data:{
        notes:notes
      }
