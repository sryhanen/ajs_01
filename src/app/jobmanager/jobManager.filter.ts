...
export function JobManagerFilter() {
  function filterContext(jobs, filterConfig) {
    const interpreter = filterConfig.interpreterFilterValue;
    const noteName = filterConfig.noteNameFilterValue;
    const isSortByAsc = filterConfig.isSortByAsc;
    ...
    if (noteName !== '') {
      filteredJobs = filteredJobs.filter((jobItem) => {
        const lowerFilterValue = noteName.toLocaleLowerCase();
        const lowerNotebookName = jobItem.noteName.toLocaleLowerCase();
